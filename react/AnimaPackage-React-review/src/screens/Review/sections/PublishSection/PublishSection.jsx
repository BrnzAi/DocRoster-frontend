  ...
  const actionButtons = [
    {
      ...
      alt: "Edit",
      iconStyles: "absolute w-[75.01%] h-[75.01%] top-[12.49%] left-[12.50%]",
    },
    {
      id: "delete",
      icon: "/img/vector-19.svg",
      alt: "Delete",
      iconStyles: "absolute w-[58.33%] h-[75.00%] top-[12.50%] left-[20.83%]",
    },
  ];

  return (
    <section
      className="flex items-center gap-6 px-6 py-3 relative self-stretch w-full flex-[0_0_auto] bg-[#ff3d000d] border-b [border-bottom-style:solid] border-[#0a17481a]"
      role="article"
      aria-label="Doctor profile card"
    >
      <img
      ...
      <div className="inline-flex flex-col items-start justify-center gap-2 relative flex-[0_0_auto]">
        {actionButtons.map((button) => (
          <button
            key={button.id}
            type="button"
            className="relative w-9 h-9 rounded-[18px] border-[0.5px] border-solid border-[#ffffff14] backdrop-blur-md backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(12px)_brightness(100%)] bg-[linear-gradient(0deg,rgba(255,61,0,1)_0%,rgba(255,61,0,1)_100%)] hover:opacity-90 active:opacity-80 transition-opacity"
            aria-label={button.alt}
          >
            <div className="top-[calc(50.00%_-_9px)] left-[calc(50.00%_-_9px)] w-[18px] h-[18px] relative aspect-[1]">
              <img
                className={button.iconStyles}
                alt={button.alt}
                src={button.icon}
              />
            </div>
